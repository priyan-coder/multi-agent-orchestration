#!/usr/bin/env python3
"""
Improved Task B - Symbol Resolver (Sharded)

Fixed global variable issues and enhanced API query strategies.
"""
...
from core_functions import (
    setup_logging, load_state, update_state_batch, resolve_symbol, 
...
def run_symbol_resolver_shard(
    ...
    try:
        # Log task start
        log_event("SYMBOL_RESOLVER_SHARD", task_id, "start", 
        ...
        if not shard_rows:
            log_event("SYMBOL_RESOLVER_SHARD", task_id, "end", "No rows to process", md_path)
            return {"shard_id": shard_id, "processed": 0, "successes": 0, "failures": 0, "elapsed": 0}
        
        print(f"🎯 Shard {shard_id}: Processing {len(shard_rows)} rows for symbol resolution")
        ...
        for row_idx in shard_rows:
            ...
            print(f"   Row {row_idx}: '{company_name}' (current symbol: '{current_symbol}')")
        ...
        for i, row_idx in enumerate(shard_rows):
            ...
            if current_symbol.strip():
                print(f"⏭️ Row {row_idx}: Already has symbol '{current_symbol}', skipping")
                continue
            
            print(f"\n🔍 Row {row_idx}: Looking up symbol for '{company_name}'")
            
            symbol = resolve_symbol(company_name, api_key)
            if symbol:
            ...
            if (i + 1) % progress_every == 0 or (current_time - last_heartbeat) >= heartbeat_seconds:
                ...
                log_event("SYMBOL_RESOLVER_SHARD", task_id, "progress", progress_msg, md_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print(f"🚀 Starting symbol resolver for shard {shard_id} with rows {shard_rows}")
