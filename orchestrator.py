...
Coordinates the complete parallel multi-agent workflow:
1. Analyzes input CSV and creates intelligent shards
2. Generates parallel task instructions for Amp execution
3. Provides system status and execution guidance

Uses simplified two-step symbol resolution (FinnHub → OpenAI)
for 100% accuracy on challenging cases.
"""
...
def generate_amp_instructions(result):
    """
    Generate structured Amp task instructions for parallel execution.

    Creates detailed task definitions for the Amp multi-agent system,
    including all necessary parameters and execution sequences.

    Args:
        result: Dictionary containing shard information from Task A

    Returns:
        Formatted instruction string for Amp execution
    """
    ...
    instructions.append(
        "Execute all of these tasks and verify that the output CSV file is generated.\n"
    )
    ...
    if result["symbol_shards"]:
        ...
        for i, shard in enumerate(result["symbol_shards"]):
            ...
            instructions.append(
                f"python task_b_symbol_resolver.py symbol_shard_{i} '{shard}'"
            )
            instructions.append("'''")
            instructions.append(")")
            instructions.append("```\n")
    ...
    if result["name_shards"]:
        ...
        for i, shard in enumerate(result["name_shards"]):
            ...
            instructions.append(f"Command:")
            instructions.append(
                f"python task_c_name_resolver.py name_shard_{i} '{shard}'"
            )
            instructions.append("'''")
            instructions.append(")")
            instructions.append("```\n")
